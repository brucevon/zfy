...
| 🔍 **全文搜索** | 服务端注入数据，客户端实时全文检索，支持标题+内容匹配 |
| 🔥 **热度地图** | 按年展示文章热力分布 |
...
```text
share/
├── home.ejs          # Trilium EJS 模板——博客入口（676 行）
├── css/blog.css      # 全部博客样式（1900 行）
└── js/blog.js        # 客户端交互脚本（820 行）

> 搜索功能自动生效：所有子笔记的标题和正文内容会在页面渲染时注入，浏览器端实时过滤匹配，无需额外配置。
```
